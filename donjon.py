...
class Salle:
    ...
    def __str__(self):
        contenu = []
        if self.monstre:
            contenu.append(f"Monstre: {self.monstre}")
        if self.tresor:
...
if __name__ == "__main__":
    donjon = creer_donjon(5, 5)
    print("Position initiale:", donjon.position_joueur)
    print("Contenu de la salle actuelle:", donjon.afficher_salle_actuelle())

    donjon.deplacer_joueur("sud")
    print("Nouvelle position:", donjon.position_joueur)
    print("Contenu de la salle actuelle:", donjon.afficher_salle_actuelle())
